...
func (w Walking) Calories() float64 {
	...
	if heightInMeters == 0 {
		return 0 // учел комментарий от ревьювера о возможном делении на ноль в коде проекта спринта 4
	}
	durationHours := w.Duration.Hours()
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedInMsec, 2)/heightInMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * durationHours * MinInHours)
}
